...
const QuoteModal = ({ isOpen, onClose, product }) => {
  ...
  const isStandardOption = product.isStandard === true && !isCustomSize;
  ...
  const handleSubmit = (e) => {
    ...
    if (isStandardOption) {
      ...
      if (isCampanaCuadrada) {
        if (formData.lado) message += `*Ancho:* ${formData.lado} cm\n`;
        if (formData.fondo) message += `*Fondo:* ${formData.fondo} cm\n`;
        if (formData.alto) message += `*Alto Campana:* ${formData.alto} cm\n`;
    ...
    <div className="quote-modal-overlay" onClick={handleOverlayClick}>
      <div className="quote-modal-content">
        <div className="quote-modal-header">
          <h2>{isStandardOption ? 'Encargar Producto' : 'Cotizar Producto'}</h2>
        ...
        <form onSubmit={handleSubmit} className="quote-form">
          {product.isStandard && (
            <div className="form-group" style={{ marginBottom: '0.5rem' }}>
              ...
              <div style={{ display: 'flex', gap: '1.5rem', marginTop: '0.5rem' }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', color: 'var(--color-text-light)' }}>
                  <input type="radio" name="sizeType" checked={!isCustomSize} onChange={() => setIsCustomSize(false)} />
                  Estándar (100x60 cm)
                </label>
                <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', color: 'var(--color-text-light)' }}>
                  <input type="radio" name="sizeType" checked={isCustomSize} onChange={() => setIsCustomSize(true)} />
                  A Medida
                </label>
              </div>
            </div>
            ...
            <>
              <div className="form-row">
                <div className="form-group">
                  <label>Ancho (Lado) (cm) *</label>
                  <input type="number" name="lado" required onChange={handleInputChange} className="form-input" />
                </div>
                <div className="form-group">
              ...
              <div className="form-row">
                <div className="form-group">
                  <label>Ancho Caño (cm) *</label>
                  <input type="number" name="anchoCano" required onChange={handleInputChange} className="form-input" />
                </div>
                <div className="form-group">
                  <label>Alto Caño (cm) *</label>
                  <input type="number" name="altoCano" required onChange={handleInputChange} className="form-input" />
                </div>
              </div>
            </>
          )}
          ...
          <div className="quote-modal-footer">
            ...
            <button type="submit" className="btn-primary" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', padding: '0.75rem 1rem', background: isStandardOption ? '#25D366' : 'var(--color-primary)' }}>
              ...
              {isStandardOption ? 'Encargar por WhatsApp' : 'Cotizar por WhatsApp'}
            </button>
          </div>
        </form>
      </div>
